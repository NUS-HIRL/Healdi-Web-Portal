...
const DAYS = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"] as const satisfies readonly DayAbbr[]
...
export const EditMedicationPage = ({
  ...
  useEffect(() => {
    setSchedule(prev => {
      const next: Schedule = { Mon: [], Tue: [], Wed: [], Thu: [], Fri: [], Sat: [], Sun: [] }
  ...
  const addDose = (day: DayAbbr) => {
    setSchedule(prev => ({ ...prev, [day]: [...(prev[day] || []), { quantity: 1, time: "" }] }))
  }
        ...
        <main className="flex-1 overflow-auto">
          <div className="px-6 py-4">
            <nav className="text-sm text-gray-500">
              Home / Select Patient / View Patient / Select Medication Plan / Add Medication Plan
            </nav>
          </div>
          ...
          <div className="px-6 pb-4">
            ...
            Edit Medication Plan</h1>
          </div>

          {/* Card */}
          <div className="px-6 pt-4 pb-6 bg-white">
            {/* Section title */}
            <h2 className="text-xl font-semibold mb-6 text-[#4FA0E8]">Existing Medication Plan</h2>
            ...
            <div className="mb-6 w-full md:flex md:items-start md:justify-between">
              ...
              <div className="space-y-4 md:space-y-0 md:flex md:items-start md:gap-8 md:flex-none">
                <div>
                  <Label className="text-sm font-semibold text-gray-500">Selected Medication</Label>
                  <p className="mt-2 text-gray-900">{medication.name}</p>
                </div>

                <div className="md:border-l md:pl-8 border-gray-200">
                  <Label className="text-sm font-semibold text-gray-500">Dosage</Label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start mb-6">
              <div className="md:col-span-1">
                <Label className="text-sm font-semibold text-gray-700">Recurring Period</Label>
                <p className="text-sm text-gray-500 mt-1 mb-5">Days to take the medication.</p>
              </div>
              <div className="md:col-span-2">
                <MultiSelect<DayAbbr>
                  value={recurringDays}
                  onChange={(next) => setValue("recurringDays", next, { shouldDirty: true })}
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-10 ">
                  ...
                  return (
                    <section key={abbr}>
                      <div className="flex items-center justify-between mb-4 ">
                        <h3 className="text-lg font-semibold text-[#4FA0E8]">{DAY_FULL[abbr]}</h3>
                        <Button
                          type="button"
                          variant="outline"
                        ...
                        <div key={`${abbr}-${idx}`} className="mb-8">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
                            <div className="md:col-span-1">
                              ...
                              <p className="text-sm text-gray-500 mt-1">Dosage of the medication.</p>
                            </div>
                            <div className="md:col-span-2 flex items-center gap-3">
                              <Input
                                ...
                                min={0}
                                value={dose.quantity}
                                onChange={(e) => updateDoseQty(abbr, idx, Number(e.target.value || 0))}
                                className="w-full"
                                placeholder="1"
                              />
                              <Button
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start mt-6">
                            <div className="md:col-span-1">
                              ...
                              <p className="text-sm text-gray-500 mt-1 mb-5">
                                Select the time at which the medication to be taken.
                              </p>
                            </div>
                            <div className="md:col-span-2">
                              <Select
                                value={dose.time || undefined}
                                onValueChange={(v) => updateDoseTime(abbr, idx, v)}
                              >
                                ...
                                <SelectContent className="max-h-64">
                                  {TIME_OPTIONS.map((t) => (
                                    <SelectItem key={`${abbr}-${idx}-${t}`} value={t}>
                                      {t}
                                    </SelectItem>
                                  ))}
                                </SelectContent>
                ...
                <div className="bg-gray-100 rounded-lg p-6 mt-2">
                  ...
                  <p className="text-sm text-gray-600 mb-4">
                    Review your filled form details and make sure everything is accurate. Once you're ready, click the
                    'Add Medication Plan' button to add the new medication plan.
                  </p>
                  <div className="flex justify-end">
                  <Button
                    className="bg-black text-white hover:bg-gray-800"
                    onClick={() => {
                      setValue("medicationId", medication.id, { shouldDirty: true })
                      router.push(`/patient-info/medication`)
                    }}
                  >
                    Add Medication Plan
